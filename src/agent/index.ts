  ...
  try {
    ...
    const normalizedVoiceId = 
      !voiceId || voiceId === 'default' || (typeof voiceId === 'string' && voiceId.trim() === '') 
        ? undefined 
        : typeof voiceId === 'string' ? voiceId.trim() : voiceId;
    ...
    if (body.agentId) {
      console.log(`⚠️  agentId provided in request body (${body.agentId}) - ignoring, will be auto-generated`);
    }
    ...
    console.log(`🎙️  Creating agent for course ${courseId} (teacherId: ${teacherId}, voiceId: ${normalizedVoiceId || 'auto-select'})`);
    ...
    const result = recreate
      ? await recreateAgentWorkflow(courseId, teacherId, normalizedVoiceId, c.env)
      : await executeAgentCreationWorkflow(courseId, teacherId, normalizedVoiceId, c.env);
