...
app.post('/api/courses/:courseId/agent', async (c) => {
  try {
    ...
    // Normalize voiceId: treat "default" as undefined to trigger auto-selection
    ...
    // Ignore agentId if provided (it's auto-generated by the system)
    if (body.agentId) {
    ...
    const result = recreate
      ? await recreateAgentWorkflow(courseId, teacherId, normalizedVoiceId, c.env.KV_CACHE, c.env.ELEVENLABS_API_KEY)
      : await executeAgentCreationWorkflow(courseId, teacherId, normalizedVoiceId, c.env.KV_CACHE, c.env.ELEVENLABS_API_KEY);
